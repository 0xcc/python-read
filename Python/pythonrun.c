...
Py_InitializeEx(int install_sigs)
{
	...
	//设置 tstate到 _PyThreadState_Current 
	...
	//在 sys module 中添加一些对象，用于import机制
    _PyImportHooks_Init();
	...
	//将site-package 加入 module 搜索 路径
    if (!Py_NoSiteFlag)
		//加载 /libs/site.py ，达到修改 sys.path的目的 
...
initmain(void)
{
    ...
    d = PyModule_GetDict(m);//获取 __main__.md_dict

	//获取d["__builtins__"] 表项
    if (PyDict_GetItemString(d, "__builtins__") == NULL) {
		//如果没有的话 ，重新加载一下，这个代码应该很少被执行吧
...
PyRun_InteractiveOneFlags(FILE *fp, const char *filename, PyCompilerFlags *flags)
{
	...
	//获取 __module__维护的PyModuleObject
    m = PyImport_AddModule("__main__");
    if (m == NULL) {
        PyArena_Free(arena);
        return -1;
    }
	//获取 __module__  __main__ 维护的md_dict
    d = PyModule_GetDict(m);
	//执行用户的输入，globals和locals 是一个东西
                  ...
                  PyObject *locals, int closeit, PyCompilerFlags *flags)
{
    PyObject *ret;
    mod_ty mod;
	//arena 用于内存分配，维护分配的原始内存和PyObject对象
    PyArena *arena = PyArena_New();
    if (arena == NULL)
        return NULL;
...
//将ast编译到byte code，并且 调用 PyEval_EvalCode，执行掉，一了百了
static PyObject *
         ...
         PyCompilerFlags *flags, PyArena *arena)
{
    PyCodeObject *co;
    PyObject *v;
	//将AST编译出字节码 PyCodeObject 对象
    co = PyAST_Compile(mod, filename, flags, arena);
    if (co == NULL)
        return NULL;
	//开始执行 PyCodeObject 对象
                     ...
                     PyArena *arena)
{
	...
	//PyParser_ParseFileFlagsEx生成CST(Concrete Syntax Tree)并将其根结点放到node*之中
