...
typedef struct {
    ...
    char *ptr;
    char *end;
	//这个strings 在写入时，指向一个PyDictObject对象，在读取出来时，指向一个PyListObjcet对象
        ...
        if (p->strings && PyString_CHECK_INTERNED(v)) {
            ...
            if (o) {
                ...
                w_byte(TYPE_STRINGREF, p);
					...
					//索引值,这个索引值在python加载pyc文件的时候需要使用,具体情况跟踪 PyMarshal_ReadObjectFromFile(FILE *fp)
                w_long(w, p);
                goto exit;
            }
            else {
				...
				//加入到dict中，如果需要再次写入一样的字符串，只需写一个索引就够了，以免pyc文件过大
                ok = o &&
...
ValueError exception is raised 鈥?but garbage data will also be written\n\
...
read (e.g. because the data has a different Python version鈥檚\n\
