...
PyObject_GetAttr(PyObject *v, PyObject *name)
{
	...
	*/
	...
	2.自定类型对象，比如 A.fun ,A为一个class
	tp->tp_getattro =type_getattro

	3.自定义类型中 定义了 __getattr__ 或则 __getattribute__ 那么  tp->tp_getattro==slot_tp_getattr_hook
		 class A(ABase):
			x=9999
			def __init__(self,x):
				self.__dict__['x']=x
			...
			def __setattr__(self,name,value):
				print "A::set:",name,value
				self.__dict__[name]=value
				print "A::set finished"
	...
	*/
	/*
	type(v)->tp_getattro(self,name)===>type(v)->type_getattro(self,name)
	*/

	/*
	
	class.__getattribute__会改写 tp_getattro方法
	*/
...
步骤
1.从 obj.__class__.dict中查找 name 得到  obtypename
2.如果 obtypename!=NULL 并且 type(obj) 定义了 __get__,__set__ ,就返回 obj.__class__.__get__()
3.从obj.__dict__中查找 name
*/
//获取属性
PyObject *
_PyObject_GenericGetAttrWithDict(PyObject *obj, PyObject *name, PyObject *dict)
{
	...
	//从 type(instance).__dict__中寻找 name
	...
	//type 是否有定义  tp_descr_get 等等
    if (descr != NULL &&
        PyType_HasFeature(descr->ob_type, Py_TPFLAGS_HAVE_CLASS)) {
		
		/*
			得到的是一个descriptor,那么需要调用这个descr->ob_type->tp_descr_get方法
			一般是 slot_tp_descr_get
		*/
		/*
			如果属性类型定义了__get__方法，
			如果属性类型没有定义 __get__方法 tp_descr_get指向member_get
		*/
        f = descr->ob_type->tp_descr_get;
		//是否有定义 __set__ ，是否是 data descriptor
        if (f != NULL && PyDescr_IsData(descr)) {
			//type( type(instance).dict[name] ).tp_descr_get !=null  and type( type(instance).dict[name] ).tp_descr_set !=null 
			//并且tp_descr_set也写了,就到 instance.__class__中去寻找这个field
...
/*
	以上逻辑和type_getattro是一样的，区别在于下面了，下面去找instance的dict的偏移了，这个就是差别了，
	类型对象的getattro 走type_getattro ，他们不需要找instance的 dict offset
	其余的需要考虑instance dict offset
	*/
	//空字典
    if (dict == NULL) {
		//从tp_dictoffset获取字典所在位置
	...
	//instance.x
    if (f != NULL) {
		//如果x 是一个 non-data descriptor 的话就走到这里，这个意思就是 type(instance.x) 有定义 __get__() 但是 没有定义 __set__(),而且 instance.__dict__['x']==null
...
//获取普通对象的attr了，从obj.__dict__中获取name
PyObject *
