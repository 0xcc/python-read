...
type_set_abstractmethods(PyTypeObject *type, PyObject *value, void *context)
{
    ...
    */
    ...
    int abstract, res;
	//if (strcmp( type->tp_name,"FrenchDeck2")==0){
	//	printf("ok\n");
	//}

    if (value != NULL) {
		//是否有数据来确定是否设置为 abstract class
...
//创建类型对象的时候会调用，type就是metaclass,一般就是这个
//如果类中指定了__metaclass__就是那个类了
static PyObject *
...
PyType_GenericAlloc(PyTypeObject *type, Py_ssize_t nitems)
{
	...
	//下面两段设置  ob_type什么的，就是那个metaclass了
...
{
	//metatype一般是PyType_Type了，args中包含了 类名、基类列表、属性\方法表
	...
	//将args中包含的 类名、基类列表、属性 分别解析到name,bases,dict三个变量中去
    ...
    */
    ...
    for (i = 0; i < nbases; i++) {
		...
		//取出基类的ob_type来与 metatype做比较
        tmptype = tmp->ob_type;
        if (tmptype == &PyClass_Type)//绕过PyClass_Type 
            continue; /* Special case classic classes */
        if (PyType_IsSubtype(winner, tmptype))//如果tmptype是metaclass的子类，不行，绕过去
            continue;
        if (PyType_IsSubtype(tmptype, winner)) {//如果metaclass是tmptype的子类，那么就选它了，为什么是这个策略啊？？？？？？？？？？？？/
            winner = tmptype;
            continue;
        }
    ...
    if (winner != metatype) {
		//说明修改过metaclass了，那么需要，重新走一遍 tp_new 的过程了
	...
	/*
		__slots__属性,如果有__slots__ 这个东西会怎样啊？？？？代码好长，不想看了
	*/
    slots = PyDict_GetItemString(dict, "__slots__");
    nslots = 0;
    add_dict = 0;
    add_weak = 0;
    may_add_dict = base->tp_dictoffset == 0;//PyBaseObject_Type->tp_dictoffset==0
    may_add_weak = base->tp_weaklistoffset == 0 && base->tp_itemsize == 0;

		
	if (slots == NULL) {
		//slots 会影响是否添加 dict的样子
		if (may_add_dict) {
            add_dict++;
        }
        if (may_add_weak) {
		...
		/*
			遍历slots中的东西，检查 当有 __dict__和 __weakref__时 ，类型对象的设置是否正确
		*/
        for (i = 0; i < nslots; i++) {
        ...
        */
		//分配list，大小为要减去add_dict 和 add_weak 也就是 -1 再 -1
        ...
        for (i = j = 0; i < nslots; i++) {
            ...
            if ((add_dict && strcmp(s, "__dict__") == 0) ||
				...
				//跳过两个特殊的
                continue;
			//字符串 __private === >_classname__private
	...
	/*
		分配类对象空间了 一般是 PyType_GenericAlloc
	*/
	...
	/*
	PyTypeObject 和 PyHeapTypeObject兼容的,这里有一个小技巧了，PyHeapTypeObject 结构的 第一部分是 PyTypeObject，
	所以类型转一下没关系，不过这个是不是也依赖编译器不要调整次序啊,不过编译器能调整次序的似乎也只有 char, short **这些类型的吧
	*/
    et = (PyHeapTypeObject *)type;
    Py_INCREF(name);
    et->ht_name = name;
	...
	/*
		这里type是一个东西 可是 之前分配的是PyHeapTypeObject对象吗，，，这里真的看不出为何这么写
	*/
...
type_getattro(PyTypeObject *type, PyObject *name)
{
	...
	/*
	先从metatype里面去找 这个name attribute
	*/
    ...
    if (meta_attribute != NULL) {
		...
		////ob_type是否有定义__get__方法，如果定义了 __get__和 __set__方法，那么就调用__get__方法了
	...
	/*
	以上逻辑和PyObject_GenericGetAttr是一样的
	*/
    /* No data descriptor found on metatype. Look in tp_dict of this
     * type and its bases */
	/*
	在本类中寻找 name这个attribute
	*/
...
/*
PyType_Type 这个对象是至关重要的，所有用户自定义的class所对应PyTypeObject 都是通过这个对象创建的，
就是所谓的元类型(metaclass)了
就是比如
int.__class__ 
输出 <type,'type'>
...
PyTypeObject PyType_Type = {
    ...
    (ternaryfunc)type_call,                     /* tp_call  创建类型对象的时候会调用*/
    0,                                          /* tp_str */
    ...
    0,                                          /* tp_descr_get */
    0,                                          /* tp_descr_set */
    offsetof(PyTypeObject, tp_dict),            /* tp_dictoffset ,tp_dict字段在类中的偏移*/
    type_init,                                  /* tp_init */
    0,                                          /* tp_alloc */
    type_new,                                   /* tp_new */
...
#define COPYBUF(SLOT) COPYSLOT(tp_as_buffer->SLOT)
    ...
    if (type->tp_as_number != NULL && base->tp_as_number != NULL) {
		...
		*/
		...
		/*当前类的nb_add 没有指定，并且 基类已经指定了 nb_add 并且(metaclass不为空，并且metaclass的nb_add 和 基类的nb_add不相等)
		为什么这么判断，这个可能涉及到成员从搜索次序了，带有空再来填坑咯
		if(!type->tp_as_number->nb_add 
			&& base->tp_as_number->nb_add!=0&&
			(basebase==NULL ||base->tp_as_number->nb_add!=basebase->tp_as_number->nb_add)){
			type->tp_as_number->nb_add=base->tp_as_number->nb_add
		}
		*/
...
PyType_Ready(PyTypeObject *type)
{
	...
	//获得基类，只有一个，似乎是基类列表中的第一个。
    base = type->tp_base;
	//如果没有指定基类，必须指定基类为PyBaseObject_Type,所以python有一条规则，所有类必须至少继承自基类
	...
	//如果基类没有初始化，先初始化基类、以是否设置tp_dict为基准
	...
	//设置metaclass,on_type域,就是那个__class__ 的结果,即metaclassyp
	...
	//把与 这个类型相关的descriptors 放入tp_dict，  如__add__ 放入 tp_dict
	...
	*/
	...
	for (i = 1; i < n; i++) {
        ...
        PyObject *b = PyTuple_GET_ITEM(bases, i);
        if (PyType_Check(b))
			//将type中没有的操作，并且type的ob_type中也没有的，而基类b中有的操作拷贝到type中去
...
//设置基类的子类列表 base->tp_subclasses.append(type)
static int
...
/*
自定义类 如果 定义了 __getattr__或则__getattribute__ 那么 type(CLASS_A)->tp_getattro指向这里
*/
...
slot_tp_getattr_hook(PyObject *self, PyObject *name)
{
	...
	/*
	python 语言规则中的__getattribute__和__getattr__的调用次序由下面代码确定了
	1.考虑__getattribute__为空 或则 __getattribute__ 是一个 PyWrapperDescrObject对象，并且这个对象的d_wrapped是PyObject_GenericGetAttr，那么调用PyObject_GenericGetAttr
	2.否则调用 __getattribute__ 得到返回值

	3.如果 __getattribute__ 返回的是空，就调用 __getattr__

	*/
    if (getattribute == NULL ||
		/*
		自定义类中没有定义__getattribute__,说明只有定义了 __getattr__ ，那么这是一个 non-data descriptor，去PyObject_GenericGetAttr那里 的逻辑会取得数据
		*/
		...
		//调用自定义的 __getattribute__
        res = call_attribute(self, getattribute, name);
        Py_DECREF(getattribute);
    }

	/*
	其实如果定义了__getattribute__，那么返回值一定不会是NULL,至少是None对象,
	除非call_attribute出错了 即Py_TYPE(getattribute)->tp_descr_get;然后这个 tp_descr_get返回NULL

	就是说 如果 __getattribute__返回值为Null 时，调用 __getattr__
	*/
...
slot_tp_descr_get(PyObject *self, PyObject *obj, PyObject *type)
{
    ...
    if (type == NULL)
        type = Py_None;
	//调用相应的__get__方法 , 此时的get为一个 PyCodeObject,是个callable对象
...
/*
创建各种对象是会调用到的 __init__方法，如果自定metaclass的话，也是从这里走
*/
static int
...
static slotdef slotdefs[] = {
	...
	/*{"__str__",offset(PyTypeObject,tp_print),NULL,NULL,"",NULL,NULL}*/

    TPSLOT("__repr__", tp_print, NULL, NULL, ""),
    TPSLOT("__getattribute__", tp_getattr, NULL, NULL, ""),

	/*{"__getattribute__",offset(PyTypeObject,tp_getattr),NULL,NULL,"",NULL,NULL}*/

    TPSLOT("__getattr__", tp_getattr, NULL, NULL, ""),
	/*{"__getattr__",offset(PyTypeObject,tp_getattr),NULL,NULL,"",NULL,NULL}*/
	//不同的操作名对应相同的操作
	...
	*/
	...
	/*{"__getattr__",offset(PyTypeObject,tp_getattro),(void *)slot_tp_getattr_hook,NULL,"",NULL,NULL }*/
   ...
   ////不同的操作名对应相同的操作
	...
	/*
		  {"__get__",offset(PyTypeObject,tp_descr_get),(void*)slot_tp_descr_get,wrap_descr_get,"descr.__get__(obj[, type]) -> value",NULL,NULL}
	*/
	...
	*/
	...
	/*
		ETSLOT("__radd__",as_number.nb_add,slot_nb_add,wrap_binaryfunc_l,"x. __radd__ (y) <==> x + y")=====>
		{
			"__radd__",
...
slotptr(PyTypeObject *type, int ioffset)
{
	...
	/*
	这里if else 有个技巧了
	在PyHeapTypeObject 中，as_sequence，as_mapping，as_number的内存 布局次序 先后为
		as_number，as_mapping，as_sequence，
	...
	*/
	...
	/*
		代码走到这里时， 
			offset 为 as_sequence 或 as_mapping 或as_number 在  PySequenceMethods,PyMappingMethods ,PyNumberMethods 对象中的起始偏移了
			ptr 为 tp_as_sequence 或 tp_as_mapping 或 tp_as_number在 PyTypeObject 对象中的其实偏移了

	*/
	//ptr+=offset 为  相应函数 在 PyHeapTypeObject中的偏移。
    if (ptr != NULL)
        ptr += offset;
    return (void **)ptr;
}
...
init_slotdefs(void)
{
    ...
    if (initialized)
        return;
	//就是把name  写回到name_strobj，这个有什么意思........
...
add_operators(PyTypeObject *type)
{
    ...
    init_slotdefs();
	//name 不为空，说明这个slot是有函数可以用的
    for (p = slotdefs; p->name; p++) {
        if (p->wrapper == NULL)
            continue;
		//函数在PyHeapTypeObject中的偏移
        ptr = slotptr(type, p->offset);
        if (!ptr || !*ptr)
            continue;

		//dict["__add__"],dict["__sub__"].....
        if (PyDict_GetItem(dict, p->name_strobj))//如果已经有了，就跳过,体现slotdefs中的优先级
            continue;
        if (*ptr == PyObject_HashNotImplemented) {
			...
			//设置descr 到 tp_dict中去，所以 tp_dict中真正有的是一个PyWrapperDescrObject
