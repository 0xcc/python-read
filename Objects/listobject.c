...
list_resize(PyListObject *self, Py_ssize_t newsize)
{
    ...
    */
	//不需要重新分配，但是新的大小比原来的1/2要大，然后减小容量了
...
author :Mike
*/
int PyList_Dump(PyListObject* pobj,FILE *f,int flags){
	return list_print(pobj,f,flags);
}
...
static void list_print_my_append(PyListObject *op, FILE *fp, int flags);
static int
list_print(PyListObject *op, FILE *fp, int flags)
{
    ...
    Py_END_ALLOW_THREADS
    Py_ReprLeave((PyObject *)op);
	list_print_my_append(op,fp,flags);
    return 0;
}

static void list_print_my_append(PyListObject *op, FILE *fp, int flags){
	printf("\nnalloced=%d\t ob_size=%d\t \n",op->allocated,op->ob_size);
