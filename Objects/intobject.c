...
PyInt_FromLong(long ival)
{
    ...
    /* Inline PyObject_New */
    v = free_list;
    free_list = (PyIntObject *)Py_TYPE(v);//ob_type指向下一个可用的PyInt_Object空间
    PyObject_INIT(v, &PyInt_Type);//设置头信息
    v->ob_ival = ival;//设置值
    return (PyObject *) v;
}
...
static void int_print_my_append(PyIntObject *v, FILE *fp, int flags);
/* ARGSUSED */
static int
     ...
     /* flags -- not used but required by interface */
{
	...
	//int_print_my_append(v,fp,flags);
    return 0;
}
...
static void int_print_my_append(PyIntObject *v, FILE *fp, int flags){
	...
	//这个last相当于系统分配的第一块PyInt_Block，存的是small int啊
