            ...
            PyObject *locals)
{
    ...
    if (code->co_zombieframe != NULL) {
		...
		//4部分构成了pyframeobject维护的动态内存区域的大小，其大小由extra确定
        ncells = PyTuple_GET_SIZE(code->co_cellvars);
        nfrees = PyTuple_GET_SIZE(code->co_freevars);
		//co_stacksize 计算栈大小
		//co_nlocals 局部变量 包括参数和局部变量
		//ncells对象 对应 co_cellvars  被嵌套函数使用的变量
		//nfrees对象 对应 co_freevars  使用外层的变量
        extras = code->co_stacksize + code->co_nlocals + ncells +nfrees;

		//new出来,之后会设置新的frame的起始状态
        if (free_list == NULL) {
		...
		//如果要取得 局部变量空间的基地址 f->f_localsplus就是了
		//如果要取得 被嵌套函数引用的的变量集合的基地址 f->f_localplus+code->co_nlocals就是
		//如果要取得 引用外层code block 的变量的集合的基地址 f->f_localplus+ code->co_nlocals +ncells 就是了

		//给各个局部变量和闭包相关的空间都设置NULL
